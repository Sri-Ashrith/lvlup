...
const gameState = {
  ...
  eventConfig: {
    ...
    levelStartTime: null, // timestamp when current level was activated
...
app.get('/api/challenges/:level/:zone', authenticateToken, (req, res) => {
  ...
  const responseData = { challenges: availableChallenges, completed };
...
app.post('/api/challenges/submit', authenticateToken, (req, res) => {
  ...
  if (lvlTimerSec && lvlStart) {
    const elapsed = (Date.now() - lvlStart) / 1000;
    if (elapsed > lvlTimerSec) {
      return res.status(403).json({ error: 'Time is up for this level.', timeExpired: true });
    }
  }
...
app.post('/api/heist/initiate', authenticateToken, (req, res) => {
  ...
  if (l3Timer && l3Start && (Date.now() - l3Start) / 1000 > l3Timer) {
    return res.status(403).json({ error: 'Time is up for this level.', timeExpired: true });
  }
...
app.post('/api/heist/compound', authenticateToken, (req, res) => {
  ...
  if (isCorrect) {
    ...
    if (heist.compoundProgress.length >= totalNeeded) {
      heist.stage = 'safe';
      // Pick 4 random puzzles from the pool to form the safe code
      const shuffledPuzzles = seededShuffle(challenges.level3.safePuzzles, heistId + heist.attackerId);
      const selectedPuzzles = shuffledPuzzles.slice(0, 4);
      const safeCode = selectedPuzzles.map(p => p.answer).join('').toUpperCase();
      heist.safeCode = safeCode;
      heist.safePuzzleIds = selectedPuzzles.map(p => p.id);
      heist.safeMaxAttempts = 6;
      heist.safeAttempts = 0;
      return res.json({ 
        success: true, 
        correct: true,
        stageComplete: true, 
...
app.post('/api/heist/safe', authenticateToken, (req, res) => {
  ...
  // L3-03: Validate against the safe code stored on the heist (4 alphanumeric chars)
  const isCorrect = answer.toUpperCase().trim() === (heist.safeCode || '').toUpperCase();
  
  heist.safeAttempts = (heist.safeAttempts || 0) + 1;
  const maxAttempts = heist.safeMaxAttempts || 6;
  ...
  if (isCorrect) {
  ...
  } else if (heist.safeAttempts >= maxAttempts) {
    // Heist failed — safe locks permanently after 24 wrong attempts
    ...
    // Wrong code — alert defender (triggers Guardian Angel availability)
    io.to(heist.targetTeamId).emit('heistWrongCode', { wrongCount: heist.safeAttempts });
    res.json({ success: true, correct: false, attemptsRemaining: maxAttempts - heist.safeAttempts });
  }
});
...
app.post('/api/admin/set-level', authenticateToken, (req, res) => {
  ...
  if (level < 0 || level > 4) return res.status(400).json({ error: 'Invalid level (0-4)' });
  gameState.eventConfig.currentLevel = level;
  gameState.eventConfig.levelStartTime = level > 0 ? Date.now() : null;
  ...
  io.emit('levelChange', { level, levelStartTime: gameState.eventConfig.levelStartTime });
  io.emit('leaderboardUpdate', getLeaderboard());
